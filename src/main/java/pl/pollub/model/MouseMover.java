...
import static pl.pollub.model.GlobalMembers.*;
...
public class MouseMover {
    ...
    private void updateMouseDeltas(float dx, float dy) {
        float frameDuration = 0.016666668F;
        ...
        this._dxFractional += pl.pollub.utils.GlobalMembers.extractFractional(this._dx);
        this._dyFractional += pl.pollub.utils.GlobalMembers.extractFractional(this._dy);
        ...
        if (Math.abs(this._dxFractional) > 1.0F) {
            ...
            this._dxFractional = pl.pollub.utils.GlobalMembers.extractFractional(this._dxFractional);
        }
        if (Math.abs(this._dyFractional) > 1.0F) {
            this._dy = (float) (this._dy + Math.floor(this._dyFractional));
            this._dyFractional = pl.pollub.utils.GlobalMembers.extractFractional(this._dyFractional);
        }
    }
